/* $Id: xpilot.c,v 4.5 1998/04/16 17:39:56 bert Exp $
 *
...
int main(int argc, char *argv[])
{
    ...
    if (Check_real_name(real_name) == NAME_ERROR) {
	xpprintf("fixing name from \"%s\" ", real_name);
    ...
    if (Check_nick_name(nick_name) == NAME_ERROR) {
	xpprintf("fixing nick from \"%s\" ", nick_name);
